...
police_rec_crime$Year <- factor(police_rec_crime$Year, levels = unique(police_rec_crime$Year), ordered = TRUE)

# Create numeric version of Year for regression
police_rec_crime$Year_num <- as.numeric(police_rec_crime$Year)

# Get unique years and select alternate ones
years <- unique(police_rec_crime$Year)
alternate_years <- years[seq(1, length(years), by = 2)]

# Create SharedData object for Crosstalk
shared_crime <- SharedData$new(police_rec_crime, group = "crime_group")

# Compute trend lines
trend_lines <- police_rec_crime %>%
  group_by(Offence) %>%
  do({
